/*
 ...
 Author      : (original author) AqD <iiiaqd@gmail.com>, modified by Myles
 <m.m.todorov@gmail.com> Version     : Description : Clevo & S76 models
 fan-control CLI & service
...
static int
ec_io_wait(const uint32_t port, const uint32_t flag, const char value);
static uint8_t ec_io_read(const uint32_t port);
static int
ec_io_do(const uint32_t cmd, const uint32_t port, const uint8_t value);
...
int main(int argc, char* argv[]) {
  ...
  if (argc <= 1) { return main_dump_fan(); }
  const char* val_str = argv[1];
  if (val_str[0] == '-' && val_str[1] == 'h') {
    printf(
        "\n\
...
\n"
  ...
  if (val == 0 && val_str[0] != '0' && val_str[1] != '\0') {
    printf("invalid fan duty %s!\n", val_str);
    return EXIT_FAILURE;
  }
  if (val == -1) {
...
static int main_ec_worker(void) {
  ...
  while (share_info->exit == 0) {
    ...
    case 0x100:
      ...
      share_info->fan_rpms =
          calculate_fan_rpms(buf[EC_REG_FAN_RPMS_HI], buf[EC_REG_FAN_RPMS_LO]);
      /*
       printf("temp=%d, duty=%d, rpms=%d\n", share_info->cpu_temp,
       share_info->fan_duty, share_info->fan_rpms);
       */
      break;
    default: printf("wrong EC size from sysfs: %ld\n", len);
    }
    close(io_fd);
    // auto EC
    if (share_info->auto_duty == 1) {
      ...
      if ((next_duty != -1 && next_duty != share_info->auto_duty_val) ||
          (next_duty == 0 && share_info->fan_duty != 0)) {
        char s_time[256];
        get_time_string(s_time, 256, "%m/%d %H:%M:%S");
        printf(
            "%s CPU=%d°C, GPU=%d°C, auto fan duty to %d%%\n",
            s_time,
            share_info->cpu_temp,
            share_info->gpu_temp,
            next_duty
...
static void main_on_sigterm(int signum) {
  ...
  if (share_info != NULL) share_info->exit = 1;
  sleep(5);
  exit(EXIT_SUCCESS);
}
...
static int ec_init(void) {
  if (ioperm(EC_DATA, 1, 1) != 0) return EXIT_FAILURE;
  if (ioperm(EC_SC, 1, 1) != 0) return EXIT_FAILURE;
  return EXIT_SUCCESS;
}
...
static int eq_fuzzy_range(int left, int right, int range) {
  ...
  if (left >= min_right && left <= max_right) return 1;
  else return 0;
}
...
static int identify_duty(int duty) {
  ...
  // default_duty); printf("could not idenfity duty returning raw input(%d)\n",
  // duty); return default_duty;
  return duty;
}
...
static int ec_auto_duty_adjust(void) {
  ...
  if (temp >= 85 && duty < 65) new_duty = 65;
  else if (temp >= 75 && duty < 40) new_duty = 40;
  else if (temp >= 65 && duty < 30) new_duty = 30;
  else if (temp >= 55 && duty < 17) new_duty = 17;

  // else if (temp <= 50 && duty > 0)
  else if (temp <= 50) new_duty = 0;
  else if (temp <= 60 && duty >= 17) new_duty = 17;
  else if (temp <= 70 && duty >= 30) new_duty = 30;
  else if (temp <= 80 && duty >= 40) new_duty = 40;
  else if (temp <= 85 && duty >= 65) new_duty = 65;
...
static int ec_query_gpu_temp(void) { return ec_io_read(EC_REG_GPU_TEMP); }
...
ec_io_wait(const uint32_t port, const uint32_t flag, const char value) {
  ...
  if (i >= 100) {
    printf(
        "wait_ec error on port 0x%x, data=0x%x, flag=0x%x, value=0x%x\n",
        port,
        data,
        flag,
        value
